...
Java_com_slingge_scanninglicense_PlateRecognition_PlateInfoRecognization(
    ...
    for (auto one:list_res) {
        ...
        if (one.confidence > 0.75) {
            ...
            //颜色 type
            plateInfo.setPlateColor();
            LOGD("Type:%d", plateInfo.getPlateType());
            jfieldID fid_plate_color  = env->GetFieldID(plateInfo_class,"plateColor","Ljava/lang/String;");
            env->SetObjectField(plateInfoObj,fid_plate_color, env->NewStringUTF(plateInfo.getPlateColor().c_str()));
